...
export const GlobalProvider = ({ children }) => {
  ...
  const response = JSON.parse(
    window.localStorage.getItem(process.env.REACT_APP_CART_NAME)
  );
  ...
  const [web3Info, web3InfoDispatch] = useReducer(
    web3InfoReducer,
    initialWeb3Info
  );
  ...
  const [userTransactions, userTxnsDispatch] = useReducer(
    transactionsReducer,
    []
  );
  ...
  const GetOrderDetails = async (buyer, orderID, chainID) => {
    try {
      ...
      const paymentDetails =
        await paymentContract.methods.getTransactionDetails(buyer, orderID);
  ...
  const searchProducts = async (searchTerm, domain) => {
    ...
    try {
      const res = await axios.post(
        `${serverHost}/api/products`,
        term,
        axiosConfig
      );
      const responseProducts = res.data.data.search_results;
      const products = responseProducts.filter(
  ...
  const getUserTransactions = async (userAddress) => {
    try {
      const response = await axios.get(
        `${serverHost}/api/order/user/${userAddress}`,
        axiosConfig
      );
  ...
  const addToCart = (productToAdd) => {
    try {
      ...
      if (productCart) {
        const index = productCart.products.findIndex(
          (product) => product.asin === productToAdd.asin
        );
        if (index !== -1) {
          return false;
        }
  ...
  const connectWallet = async (chainid = "") => {
    ...
    const web3Modal = new Web3Modal({
      network:
        window.detectedProvider.chainId &&
        getNetwork(window.detectedProvider.chainId),
      cacheProvider: true,
      providerOptions: getProviderOptions(),
    });
