...
import {
  ...
  initShipAddrErr,
  initShipAddrErrMsg,
} from "../../Helpers/initsShippingAddr";
...
export default function Bill() {
  // Global state
  const { cart, supportedNet, web3Info, deleteCart } =
  ...
  const checkShippingDetails = () => {
    const { isError, updatedShipAddrErr, updatedShipAddrErrMsg } =
  ...
  const handleChange = async (event) => {
    ...
    setTokenDecimals(
      supportedTokens[web3Info.chainID][Number(event.target.value) - 1].decimals
    );
  ...
  const handlePayment = async () => {
    try {
      ...
      if (
        Number(calculate(chargePercent, taxPercent, cart.totalPrice)) >
      ...
      const chainID = web3.utils.isHex(chainId)
      ...
      const totalPriceBN = new BigNumber(
        calculate(chargePercent, taxPercent, cart.totalPrice)
      ).times(BIG_TEN.pow(tokenDecimals));
      ...
      const data = await paymentContract.methods
        .makePayment(orderID, tokenIndex, totalPriceBN.toString(), totalQty)
        .send({
      ...
      await awaitBlockConsensus(
        [web3Info.web3],
        txHash,
        1,
        750,
        async (error, txnReceipt) => {
          try {
            if (error) {
              console.log(error);
              // return false;
            ...
            paymentContract.methods
              .getTransactionDetails(web3Info.address, orderID)
              .call()
  ...
  const handleApproval = async () => {
    try {
      ...
      if (
        ...
        const tokenAddress =
          supportedTokens[web3Info.chainID][tokenIndex - 1].address;
        const paymentAddr = paymentAddresses[web3Info.chainID];
        const tokenContract = new web3Info.web3.eth.Contract(
          tokenABI,
          tokenAddress
        );
        // BIG_TEN
        ...
        const data = await tokenContract.methods
          .approve(paymentAddr, ethers.constants.MaxUint256)
          .send({
            gas: "47000",
            gasPrice: "150",
            from: web3Info.address,
      ...
      if (
        ...
        const allowance = new BigNumber(
          await tokenContract.methods
            .allowance(web3Info.address, paymentAddr)
            .call()
        );
        const totalPriceBN = new BigNumber(
          cart.totalPrice * 10 ** tokenDecimals
        );
    ...
    try {
      if (
        ...
        const tokenAddress =
          supportedTokens[web3Info.chainID][tokenIndex - 1].address;
        const tokenDecimals =
          supportedTokens[web3Info.chainID][tokenIndex - 1].decimals;
        ...
        const tokenBal = await tokenContract.methods
          .balanceOf(web3Info.address)
          .call();
        const tokenBalance = Number(
          (parseFloat(tokenBal) / 10 ** tokenDecimals).toFixed(10)
        );
    ...
    <Hero>
      <div className="bill">
          ...
          <div className="input__row">
            <div className="input__outer">
              ...
              <input
                type="email"
                style={{
                  border: shippingAddrError.email ? "1px solid red" : "none",
                }}
                name="email"
                ...
                placeholder="Email"
              />
              <small style={errMsgStyle}>
                {shippingAddrError.email ? shipAddrErrMsg.email : ""}
              </small>
            </div>
            <div className="input__outer">
              <label htmlFor="phone">
                Phone number
                <small
                  style={{ opacity: "0.8", color: "black", marginLeft: "15px" }}
                >
                  (e.g. +2348123456789)
                </small>
              </label>
              ...
              />
              <small style={errMsgStyle}>
                {shippingAddrError.phone ? shipAddrErrMsg.phone : ""}
              </small>
            </div>
            <div className="input__outer">
              ...
              <input
                type="text"
                style={{
                  border: shippingAddrError.street ? "1px solid red" : "none",
                }}
                name="street"
              ...
              />
              <small style={errMsgStyle}>
                {shippingAddrError.street ? shipAddrErrMsg.street : ""}
              </small>
            </div>
            <div className="input__outer">
              ...
              <input
                type="text"
                style={{
                  border: shippingAddrError.city ? "1px solid red" : "none",
                }}
                name="city"
              ...
              />
              <small style={errMsgStyle}>
                {shippingAddrError.city ? shipAddrErrMsg.city : ""}
              </small>
            </div>
            <div className="input__outer">
              ...
              <input
                type="text"
                style={{
                  border: shippingAddrError.state ? "1px solid red" : "none",
                }}
                name="state"
              ...
              />
              <small style={errMsgStyle}>
                {shippingAddrError.state ? shipAddrErrMsg.state : ""}
              </small>
            </div>
            <div className="input__outer">
              ...
              <input
                type="text"
                style={{
                  border: shippingAddrError.country ? "1px solid red" : "none",
                }}
                name="country"
              ...
              />
              <small style={errMsgStyle}>
                {shippingAddrError.country ? shipAddrErrMsg.country : ""}
              </small>
            </div>
            <div className="input__outer">
              ...
              <input
                type="text"
                style={{
                  border: shippingAddrError.postalCode
                    ? "1px solid red"
                    : "none",
                }}
                name="postalCode"
              ...
              />
              <small style={errMsgStyle}>
                {shippingAddrError.postalCode ? shipAddrErrMsg.postalCode : ""}
              </small>
            </div>
          </div>
        </form>
        <div style={{ color: "black", margin: "15px 5px 25px 5px" }}>
          {cart != null && cart.totalPrice != null && cart.totalPrice !== 0 && (
            <h6>
              Total Price: $
              {Number(
                calculate(chargePercent, taxPercent, cart.totalPrice)
        ...
        <div style={payStyling}>
              ...
              <>
                <Box sx={{ maxWidth: 140, minWidth: 130 }}>
                  <FormControl fullWidth>
                    ...
                    >
                      {supportedNet &&
                        supportedTokens[web3Info.chainID].map((token) => (
                          <MenuItem
                            sx={menuItemStyle}
                            key={token.index}
                            value={token.index}
          ...
          <h6 style={balanceStyling}>
            {tokenIndex !== null
              ? "Balance: $" + Number(userBalance).toFixed(3)
              : ""}
          </h6>
        </div>
        <div className="bill__footer">
          {web3Info.connected === undefined ? (
            <h6 style={{ color: "#123456", fontWeight: "600" }}>
            ...
            <div className="approve-pay-btns">
              {!isApproved && (
                ...
                >
                  {isApprovalLoading ? "Approving..." : "Approve Payment"}
                </button>
              )}
              <button
                disabled={!isApproved || isPaymentLoading}
                onClick={handlePayment}
                className="button primary bill-pay-btn"
              >
